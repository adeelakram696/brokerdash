...
// Batch wrapper functions for incremental data fetching
export const getAllLeadsAssignedBatch = async (dates, users) => {
  const result = await getAllLeadsAssigned(dates, users);
  return result;
};

export const getAllAssignedLeadsDealsBatch = async (dates, employees) => {
  // This function doesn't use dates in original, but we accept it for consistency
  const result = await getAllAssignedLeadsDeals(employees);
  return result;
};

export const getDealFundsBatch = async (dates, employees) => {
  // Swap parameter order for consistency
  const result = await getDealFunds(employees, dates);
  return result;
};

export const getDisqualifiedLeadsDealsBatch = async (dates, employees) => {
  // Swap parameter order for consistency
  const result = await getDisqualifiedLeadsDeals(employees, dates);
  return result;
};

export const getTeamTotalActivitiesBatch = async (
  duration,
  actionIds,
  employees,
) => {
  const result = await getTeamTotalActivities(duration, actionIds, employees);
  return result;
};
